<?php
    ...
    // Consulta
    $consulta = $conexion->query("SELECT * FROM RUTA");


   if($consulta->num_rows >0){
       $delimiter = ";"; //En caso de problemas, cambiar el delimitador
       $nombreArchivo = "rutas".date('Y-m-d').".csv";

       //Creacion de un apuntador al fichero
       $fp = fopen('php://memory', 'w');

       //Establecer el nombre de columnas de archivo
       $cabeceras = array('idItinerario','nombreItinerario','contenido','idPais','nombrePais' );
       fputcsv($fp,$cabeceras,$delimiter);

       //Poner resultado de consulta

       while ($filas = $consulta->fetch_assoc()) {
         $linea = array($filas['idItinerario'],$filas['nombreItinerario'], $filas['contenido'],
             $filas['idPais'],$filas['nombrePais'] );
