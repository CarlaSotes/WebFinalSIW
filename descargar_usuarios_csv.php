...
if($consulta->num_rows >0){
    ...
    $nombreArchivo = "usuarios".date('Y-m-d').".csv";

    //Creacion de un apuntador al fichero
    $fp = fopen('php://memory', 'w');

    //Establecer el nombre de columnas de archivo
    $cabeceras = array('idUsuario','nombre','apellido','correo','contraseña', 'rol');
    fputcsv($fp,$cabeceras,$delimiter);

    //Poner resultado de consulta

    while ($filas = $consulta->fetch_assoc()) {
        $linea = array($filas['idUsuario'],$filas['nombre'], $filas['apellido'],
            $filas['correo'],$filas['contraseña'], $filas['rol']);
